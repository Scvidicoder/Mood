...
import type { OrderStatus, StaffOrderSummary } from "../../../types/orders";
...
type DialogState =
  | { type: "confirm" | "reject" | "time"; order: StaffOrderSummary }
  | null;
...
export function StaffOrdersPage() {
  ...
  function openDialog(type: NonNullable<DialogState>["type"], order: StaffOrderSummary) {
    ...
    <section>
      <div className="staff-page-heading">
        <div>
          ...
          <h1>Staff orders</h1>
          <p>Pending orders are shown newest first and refresh automatically.</p>
        </div>
      ...
      {dialog ? (
        <div aria-labelledby="staff-order-dialog-title" aria-modal="true" className="modal-backdrop" role="dialog">
          <form className="staff-order-dialog" onSubmit={submitDialog}>
            ...
            <h2 id="staff-order-dialog-title">
              {dialog.type === "reject" ? "Reject order" : dialog.type === "confirm" ? "Confirm order" : "Change estimated ready time"}
            </h2>
            {dialog.type === "reject" ? (
            ...
            <div className="staff-order-actions">
              ...
              <button className={`button ${dialog.type === "reject" ? "button-danger" : ""}`} disabled={activeMutation.isPending} type="submit">
                {activeMutation.isPending ? "Saving…" : dialog.type === "reject" ? "Reject order" : dialog.type === "confirm" ? "Confirm order" : "Save time"}
              </button>
            </div>
          </form>
        </div>
